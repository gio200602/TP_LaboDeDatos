...
localidades_principal = sql^"""SELECT  categoria,centroide_lat,centroide_lon, departamento_id, departamento_nombre, municipio_id,
                               municipio_nombre, id AS localidad_id, nombre AS localidad_nombre
                               FROM localidades"""
#Ponemos todos los nombres en mayúsculas
localidades_principal = sql^"""SELECT categoria,centroide_lat,centroide_lon,departamento_id,
...
cod_departamento = pd.read_csv("/home/axel/Documentos/LaboDeDatos/Tp1/diccionario_cod_depto.csv")
departamentos = sql ^  """
                 SELECT DISTINCT codigo_departamento_indec AS departamento_id, nombre_departamento_indec AS departamento_nombre
                 FROM cod_departamento
                """
#Ponemos todos los nombres en mayúsculas
departamentos = sql^"""SELECT departamento_id, UPPER(departamento_nombre) AS departamento_nombre
                            FROM departamentos """
...
#%%
#Creamos la tabla que relaciona municipios y departamentos
municipio_a_departamento = sql^"""SELECT municipio_id, municipio_nombre, departamento_id,
                            departamento_nombre
                            FROM localidades_principal"""
                            
municipio_a_departamento.to_csv('municipio_a_departamento')

#Lo mismo para localidades y departamentos
localidad_a_departamento = sql^"""SELECT localidad_id,localidad_nombre,departamento_id,
                            departamento_nombre
                            FROM localidades_principal
                           """
...
porque con el id es suficiente y evitamos así redundancia""" 
padron = pd.read_csv("/home/axel/Documentos/LaboDeDatos/Tp1/padron-de-operadores-organicos-certificados.csv", encoding="1252")
...
productos=sql^"""
                    ...
                    FROM padron
                    WHERE establecimiento!='NC' AND establecimiento IS NOT NULL AND razon_social IS NOT NULL
                    ORDER BY razon_social,establecimiento
                    """

#Separamos los productos por coma, ya que si no no cumple con la 1FN (valores multivaluados)
productos["productos"]=productos["productos"].str.split(",")

padron=padron.drop(['productos'], axis = 1)

padron = sql^"""
                    SELECT DISTINCT provincia_id,departamento,REPLACE(rubro, 'AGICULTURA', 'AGRICULTURA') as rubro,
                                    categoria_id,Certificadora_id,razon_social,establecimiento
                    FROM padron
                    WHERE establecimiento!='NC' AND establecimiento IS NOT NULL AND razon_social IS NOT NULL AND rubro IS NOT NULL
                    ORDER BY provincia_id,departamento,rubro,categoria_id,Certificadora_id,razon_social,establecimiento
                    """
...
son_departamentos = sql^"""SELECT provincia_id, departamento, rubro,
                categoria_id, Certificadora_id, razon_social, establecimiento
                FROM padron
                INNER JOIN departamentos
                ON padron.departamento = departamentos.departamento_nombre"""
son_municipios = sql^"""SELECT  provincia_id, departamento, rubro,
                categoria_id, Certificadora_id, razon_social, establecimiento
                FROM padron
                INNER JOIN municipios
                ON padron.departamento = municipios.municipio_nombre
                """
                


                 
son_localidades = sql^"""SELECT  padron.departamento AS nombre
                FROM padron
                INNER JOIN localidades_principal
                ON padron.departamento = localidades_principal.localidad_nombre"""
...
padron_final_1 = sql^"""SELECT *
                ...
                INNER JOIN localidad_a_departamento
                ON departamento = localidad_a_departamento.localidad_nombre"""
padron_final_1 = sql^"""SELECT provincia_id, departamento_nombre AS departamento, rubro,
                categoria_id, Certificadora_id, razon_social, establecimiento
                FROM padron_final_1"""
padron_final_2 = sql^"""SELECT *
                FROM padron_sin_departamentos
                INNER JOIN municipio_a_departamento
                ON departamento = municipio_a_departamento.municipio_nombre"""
padron_final_2 = sql^"""SELECT provincia_id, departamento_nombre AS departamento, rubro,
                categoria_id, Certificadora_id, razon_social, establecimiento
                FROM padron_final_2"""                
padron_final = sql^"""SELECT * FROM padron_final_1
                      UNION
...
localidades_censales = pd.read_csv("/home/axel/Documentos/LaboDeDatos/Tp1/Entrega/localidades-censales.csv")
padron = pd.read_csv("/home/axel/Documentos/LaboDeDatos/Tp1/Entrega/padron-de-operadores-organicos-certificados.csv", encoding="windows-1252")
...
actividades = sql ^ """
                ...
                """
#Relacionamos las tablas de rubros y actividades mediante sus ids para no perder la relacion original
rubro_actividad = sql ^ """
                 SELECT DISTINCT clae2 AS rubro_id, CASE WHEN letra IS NULL THEN 'Z' ELSE letra END AS actividad_id
                 FROM clae2
                 ORDER BY rubro_id
                """

#Asi se recuperaria la relacion original

relacion_original1 = sql ^ """
                 SELECT DISTINCT ru.rubro_id, ru.rubro, a.actividad_id, a.actividad
                 FROM actividades AS a, rubro_actividad AS r, rubros AS ru
                 WHERE a.actividad_id=r.actividad_id AND ru.rubro_id=r.rubro_id
                 ORDER BY ru.rubro_id
                """
...
#Relacionamos las tablas de provincias y departamentos con sus ids para no perder la relacion original
...
relacion_original2 = sql ^ """
                 SELECT DISTINCT d.departamento_id, d.departamento_nombre, p.prov_id, p.prov_nombre
                 FROM provincias as p, departamentos as d, prov_departamento as pr
                 WHERE p.prov_id=pr.prov_id AND d.departamento_id=pr.departamento_id
...
salarios = sql ^  """
                 SELECT DISTINCT fecha,codigo_departamento_indec AS departamento_id, clae2 AS rubro,
                 w_median AS salario_mediano
                 FROM median
                 WHERE salario_mediano>0 AND departamento_id IS NOT NULL
                 ORDER BY fecha, departamento_id, rubro, salario_mediano
                """

#Asi se recuperaria la relacion original

#relacion_original3 = sql ^ """
                #  SELECT DISTINCT fecha, departamento_id, prov_id as prov, rubro, salario_mediano
                #  FROM salarios as s
                #  INNER JOIN prov_departamento as d
                #  ON d.departamento_id=s.departamento
                #  ORDER BY fecha, departamento, prov, rubro, salario_mediano
                # """
...
salarios_con_provincias = sql ^ """
                 SELECT DISTINCT fecha,s.departamento_id, prov_nombre,rubro,salario_mediano
                 ...
                 ORDER BY fecha,s.departamento_id,prov_nombre,rubro,salario_mediano
                """
...
salario_promedio_2014 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro, AVG(salario_mediano) AS promedio2014
                 FROM salarios2014
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """

salario_promedio_2015 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro,AVG(salario_mediano) AS promedio2015
                 FROM salarios2015
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """
salario_promedio_2016 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro,AVG(salario_mediano) AS promedio2016
                 FROM salarios2016
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """
salario_promedio_2017 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro, AVG(salario_mediano) AS promedio2017
                 FROM salarios2017
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """
...
salario_promedio_2018 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro, AVG(salario_mediano) AS promedio2018
                 FROM salarios2018
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """
salario_promedio_2019 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro, AVG(salario_mediano) AS promedio2019
                 FROM salarios2019
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """
...
salario_promedio_2020 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro, AVG(salario_mediano) AS promedio2020
                 FROM salarios2020
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """
...
salario_promedio_2021 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro, AVG(salario_mediano) AS promedio2021
                 FROM salarios2021
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """
...
salario_promedio_2022 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro, AVG(salario_mediano) AS promedio2022
                 FROM salarios2022
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """
...
salario_promedio_2023 = sql ^  """
                 SELECT DISTINCT prov_nombre,rubro, AVG(salario_mediano) AS promedio2023
                 FROM salarios2023
                 GROUP BY prov_nombre,rubro
                 ORDER BY prov_nombre,rubro
                """
...
padron = pd.read_csv("padron-de-operadores-organicos-certificados.csv", encoding="1252")
#print(padron)
padron=padron.drop(['pais_id'], axis = 1)
padron=padron.drop(['pais'], axis = 1)
padron=padron.drop(['provincia'], axis = 1)
...
#print(padron)
certificadores = sql^"""
                    SELECT DISTINCT Certificadora_id AS certificadora_id, certificadora_deno
                    FROM padron
                    ORDER BY certificadora_id
                    """
#print(certificadores)
...
print(cod_depto)
departamentos= sql^"""
                 SELECT DISTINCT codigo_departamento_indec AS depto_id, nombre_departamento_indec AS depto_nombre
                 FROM cod_depto
                 ORDER BY depto_id
                """
print(departamentos)
...
print(provincias)
depto_a_prov = sql^"""
                 SELECT DISTINCT codigo_departamento_indec AS depto_id,id_provincia_indec AS prov_id
                 FROM cod_depto
                 ORDER BY depto_id
                """
print(depto_a_prov)
...
print(provincias_sin_operadores1)
#Consultas punto b)
departamentos_de_padron =sql^"""
                    SELECT DISTINCT departamento
                    FROM padron
                    """
print(departamentos_de_padron)

departamentos_sin_operadores = sql^"""
                 SELECT DISTINCT UPPER(depto_nombre) as departamentos
                 FROM departamentos
                 EXCEPT
                 SELECT DISTINCT departamento
                 FROM padron
...
actividad_con_mas_operadores =sql^"""
                ...
                """
print(actividad_con_mas_operadores)

#Consultas punto d)
clae2 = pd.read_csv("diccionario_clae2.csv")
median = pd.read_csv("w_median_depto_priv_clae2.csv")
...
rubro_actividad= sql^"""
                 ...
                 ORDER BY rubro_id
                """
print(rubro_actividad)
#ACLARAR QUE ASUMIMOS O INTERPRETAMOS QUE GANADERIA(ACTIVIDAD CON MAS OPERADORES) ESTA INCLUIDA EN RUBRO_id=1
salarios= sql^"""
                 SELECT DISTINCT fecha,codigo_departamento_indec AS depto, clae2 AS rubro, w_median AS salario_mediano
                 FROM median
                 WHERE salario_mediano>0 AND depto IS NOT NULL
                 ORDER BY fecha, depto, rubro, salario_mediano
                """
print(salarios)
...
agricultura_promedio_anual= sql^"""
                 ...
                 FROM salarios
                 WHERE fecha LIKE '2022%' AND rubro=1
                 GROUP BY fecha
                 ORDER BY fecha DESC
                """
#Para acceder al mas reciente usamos la funcion head(1) que nos devuelve el primer valor
       
print(agricultura_promedio_anual.head(1))
